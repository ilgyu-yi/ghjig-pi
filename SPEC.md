...
**Column shape.** The placement column carries three slots — `home:` the layer the rule selects, with the instrument holding it today or the fact that none does yet; `backstop:` §3.11's unbypassable home, `same` where it coincides with the home, `deferred` where no unbypassable layer evaluates the class's predicate yet and §3.11's amortization carries the gap, or `none` on one of the two reasons §3.11 admits: that none is owed, or that one is owed and no unbypassable layer ever sees the guarded act; `earlier:` the call sites and advisory echoes sitting beneath the home — a call site being either a pre-image of the home's gate or a call site of the class's own predicate over which §3.11's by-construction non-divergence does not hold, and an echo being one that is live or one recorded as planned. The deciding-information column carries two — `supplies:` the layer the deciding fact comes from exactly, and `infers:` the nearest layer the rule disqualifies. Where a class's arms differ inside a slot, in either column, that slot is written as `arm: value` pairs separated by `;`; where the arms agree it carries the single value. An empty slot is written `—`, and a cell writes that character for nothing else, so an omission renders as a short cell **in the row** rather than vanishing from a list. No check enforces this shape mechanically today: the container makes an omission visible, it does not reject one, and the shape is enforced at review (§2.3). Slots stay short — a slot that needs a reason gives the reason in a few words and routes the derivation to the prose beneath the table, because a slot running to several hundred characters defeats the column it sits in.
